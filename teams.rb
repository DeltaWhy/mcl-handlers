  ...
  class Teams < Mcl::Handler
    ...
    def usage(player)
      d = [["setup", "create default teams"],
           ...
           ["leave [<player> ...]", "leave or remove players from a team"]]
      d.each do |cmd, desc|
    ...
    def register_teams acl_level, acl_levels
      register_command :teams, desc: "Team setup helpers", acl: acl_level do |player, args|
        ...
        when "leave"
          acl_verify(player, acl_levels[:leave])
          if args.length == 1
            $mcl.server.invoke "/scoreboard teams leave #{player}"
          else
            acl_verify(player, acl_levels[:leave_others])
            args[1..args.length].each do |p|
              $mcl.server.invoke "/scoreboard teams leave #{p}"
            end
          end
        else
          usage(player)
